...
RSpec.describe Channel::Driver::Sms::Massenversand do
  it 'passes' do
    ...
    api = channel.driver_instance.new channel.options
    expect(api.send(receiver_number, message_body)).to be true
  end
  ...
  it 'fails' do
    ...
    api = channel.driver_instance.new channel.options
    expect { api.send(receiver_number, message_body) }.to raise_exception(RuntimeError)
  end
